...
def get_player_page_url_list():
    #投手
    endpoint = 'http://www.baystars.co.jp/players/category/pitcher'
    #捕手
    #endpoint = 'http://www.baystars.co.jp/players/category/catcher'
    #内野手
    #endpoint = 'http://www.baystars.co.jp/players/category/infielder'
    #外野手
    #endpoint = 'http://www.baystars.co.jp/players/category/outfielder'
    ...
    url_list = []
    #選手の詳細ページのURLを抜き取る
    for player in html.find_all(attrs={"class":"clearfix"}):
        for a_tag in player.find_all("a"):
            url =  a_tag.get("href")
            if url != None and url.find("detail") != -1:
                url_list.append("http://www.baystars.co.jp" + url)
    #デバック
...
def get_player_image_url(player_page_url):
    ...
    #画像のURLを抜き取る
    for image in html.find_all(attrs={"class":"playerPhoto"}): 
...
def get_player_data(player_page_url):
    ...
    player_data = []
    #選手名を抜き取る
    for title in html.find("title"): 
        player_name = re.search(r'(.+)　｜　横浜DeNAベイスターズ', title)
...
def get_image_name(img_url):
    img_name = re.search(r'https://s3-ap-northeast-1.amazonaws.com/ydbofficial-prod/img/players/2017/(.+).jpg', img_url)
...
def convert_to_dic(player_data, file_name):
    #辞書を準備
    player_dic = {}
    key_list = ["name", "birthday", "age", "year", "home", "height", "weight", "pb", "draft"]
